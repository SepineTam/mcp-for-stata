...
2. Email security concerns to: sepinetam@gmail.com
3. Include a detailed description of the vulnerability
4. Allow reasonable time for response and resolution

### Reporting Requirements

We only accept security reports that have been manually verified and can be reproduced.
Please do **not** submit bulk reports generated by AI tools without human validation.
If a vulnerability has already been fixed in the latest release, do not report it.
If a previous report was closed as `not planned` or `invalid`, do not resubmit it or create duplicate issues.
...
1. **不要**公开提交issue
2. 通过邮件报告安全问题：sepinetam@gmail.com
3. 包含漏洞的详细描述
4. 给予合理的响应和解决时间

### 报告要求

我们只接受经过人工核实且可复现的安全漏洞报告。
请勿提交由 AI 工具批量生成、未经人工验证的报告。
如果该漏洞已在最新版本中修复，请勿重复报告。
如果之前的报告已被标记为 `not planned` 或 `invalid`，请勿重复提交或换号提交。
