...
<details open>
<summary>Click to expand v1.25.0 details</summary>

## [1.25.0] - 2026-09-18

### Added
- **GitHub Addons**: Add `install --with-addon` / `--no-addon` (default: on) for supported components from `plugins/stata-toolbox`, and `--with-extra` / `--no-extra` (default: off) for `plugins/external`
- **Component Management**: Resolve a GitHub ref to one immutable commit, download complete skill directories, and track SHA-256 ownership to preserve locally modified files and configuration entries; select a snapshot with `--addon-ref`
- **Research Extras**: Include optional DIP, NBER and Zotero MCP definitions with client-specific configuration formats
- **Claude Components**: Install writing rules, commands and agents where supported, and configure available language servers through a managed Claude Code LSP plugin without installing their executables
- **Installation Guides**: Add English and Chinese installation guides covering uvx, scripts, native plugins, manual setup, local packages, executables, source checkouts and verification
- **Installer Verification**: Add client/platform configuration tests and opt-in native Codex/Claude acceptance tests using isolated user directories

### Fixed
- **Client Installation**: Deduplicate aliases in `--all`, continue addon setup when the base MCP entry already exists, and prevent client-specific launch settings from leaking to subsequent targets
- **Configuration Paths and Formats**: Respect `CODEX_HOME`, `CLAUDE_CONFIG_DIR` and OpenCode's `XDG_CONFIG_HOME`; correct OpenCode environment fields and Cline/OpenClaw HTTP transport settings
- **Registry Metadata**: Align multilingual README MCP identifiers with `server.json` and validate release metadata before publication
- **Binary Packaging**: Use the current CLI entry point and exclude dependency test suites from release executables

### Security
- **URL Configuration**: Preserve system/user URL-security settings over project overrides for the existing beta configuration keys
- **Addon Installation**: Reject unsafe remote paths and symbolic links, bound downloads, validate Git blobs, back up configuration changes, and serialize ownership-manifest updates

### Upgrade Notes
- `install` now requests Stata addons by default. Use `--no-addon` for base MCP configuration only; extras remain opt-in.
- Component support varies by client. Unsupported components are skipped, and optional download failures do not undo base MCP setup. Hooks and Hermes/DSH optional components are not enabled by this installer.
- Addons follow GitHub `master` by default, independently of the Python package version. Use `--addon-ref` to select a tag or commit.
- External services and language servers still require their own prerequisites. Installation checks do not replace verifying a live Stata analysis in the client.
