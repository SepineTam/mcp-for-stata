...
1. **授权**：MCP 要求客户端批准，CLI 在未提供 `-y` 或 `--yes` 时进行询问；
   GitHub 还要求精确仓库白名单
2. **校验**：执行前再次校验包名和来源
3. **Stata 执行**：向 Stata 发送已校验的安装命令
4. **结果验证**：Unix 下，Controller 正常返回即视为成功，因为 Stata 返回码错误、
   超时和会话异常退出都会抛出异常。Windows 下保守校验安装日志；GitHub 安装必须
   出现明确的终态成功文本，并且任何错误信号都会使其判定失败。
