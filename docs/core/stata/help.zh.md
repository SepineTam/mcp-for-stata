...
- **全局缓存**：将帮助文件存储在 `~/.statamcp/help/` 中以便跨项目重用
...
1. **命令校验**：在访问缓存或 Stata 前，先清理并校验命令名
2. **缓存检查**：从已启用的项目缓存和全局缓存中选择最新的非空结果
3. **Stata 查询**：如果未缓存，向 Stata CLI 发送 `help {command}` 请求
4. **文档检索**：Stata 在其本地文档中搜索指定命令
5. **缓存存储**：将结果保存到缓存（如果启用）
6. **结果返回**：返回帮助文本以供显示或处理

使用 `replace=True` 可以跳过缓存检索、实时查询 Stata，并覆盖项目缓存和全局缓存。系统没有基于 TTL 的自动刷新。
