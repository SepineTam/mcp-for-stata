...
SMCL（Stata 标记与控制语言）日志中可能包含 `{space N}` 和 `{hline N}` 等标签，它们会展开为重复的空格或横线。如果日志被篡改或包含异常大的 `N`，解析器在将 SMCL 转换为纯文本时可能会分配过多内存。

为避免这种情况，SMCL 解析器对每个标签的展开数量施加了硬性上限：

- **最大展开长度**：每个标签 10,000 个字符
- **受影响的标签**：`{space N}`、`{hline N}`
- **默认回退**：不带数字的 `{hline}` 默认展开为 13 个横线

该限制在通过 `read_log` 工具读取 SMCL 日志，或通过 `--enable-smcl true` 启用 SMCL 输出时自动生效。此限制不可配置，且独立于 RAM 监控器，但目标相同，都是防止资源耗尽。
