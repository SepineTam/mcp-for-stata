...
| `enable_data_info_url_guard` | Boolean | `false` | None | Enables allowlist checks for URL data sources passed to `get_data_info`. |
...
`enable_data_info_url_guard` controls whether URL data sources passed to `get_data_info` must match an allowlist.
...
When this switch is `false`, URL sources still go through the baseline URL checks: HTTPS only, no IP hosts, and no URL userinfo. When it is `true`, the URL hostname must also match `data_info_allowed_url_domains`.

## `data_info_allowed_url_domains`

`data_info_allowed_url_domains` is the hostname allowlist used by the `get_data_info` URL guard.
