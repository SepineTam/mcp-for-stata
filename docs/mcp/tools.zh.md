...
- `data_path`：数据文件的本地路径或 URL（必填）
  ...
  - URL 数据源必须使用 HTTPS，不能使用 IP 地址主机，也不能包含 URL userinfo
  - 当 `[BETA] enable_data_info_url_guard=true` 时，URL 主机名还必须命中 `data_info_allowed_url_domains`
...
# 远程数据获取；除非 enable_data_info_url_guard=true，否则域名白名单默认关闭
